...
- ⚠️ **Partial**: Partially implemented or works in limited scenarios
...
Given that this SDK is designed for **microservices-to-microservices communication**, here are recommendations for working within current limitations:

### ✅ **Recommended Patterns (Fully Supported)**
...
1. **Standard CRUD Operations**
   ...
   ```
   ✅ All these patterns work perfectly with the SDK.
...
2. **Pagination and Filtering**
   ...
   ```
   ✅ Simple query parameters are fully supported.

3. **Nested Objects in Request/Response**
   ```java
   @PostMapping("/orders")
   ...
   public class OrderDto {
       private Long id;
       private CustomerDto customer;  // Nested DTO
       private List<OrderItemDto> items;  // Collections
   }
   ```
   ✅ Complex nested DTOs are fully supported.

### ⚠️ **Patterns Requiring Workarounds**

1. **Authentication/Authorization Headers**
   - **Problem:** `@RequestHeader` not supported
   - **Workaround:** Use RestClient interceptors for service-to-service tokens
   ```java
   @Bean
   public ClientHttpRequestInterceptor authInterceptor() {
       return (request, body, execution) -> {
           request.getHeaders().add("X-Service-Token", tokenProvider.getToken());
...
2. **Distributed Tracing/Correlation IDs**
   - **Problem:** Cannot pass correlation headers per method
   - **Workaround:** Use interceptor to propagate from MDC/ThreadLocal
   ```java
   @Bean
...
3. **Multi-valued Query Parameters (IDs, Tags, etc.)**
   - **Problem:** `@RequestParam List<Long> ids` not supported
   - **Workaround:** Use comma-separated strings
   ```java
   // Instead of: List<Long> ids
   @GetMapping("/users")
   ResponseEntity<List<UserDto>> getUsersByIds(@RequestParam String ids);
   
   // Calling service parses: "1,2,3,4"
   ```
   - **Alternative:** Use POST with request body for complex filters
   ```java
   @PostMapping("/users/search")
   ResponseEntity<List<UserDto>> searchUsers(@RequestBody UserSearchRequest request);
...
4. **Optional Parameters with Defaults**
   - **Problem:** `@RequestParam(defaultValue = "10")` ignored
   - **Workaround:** Always pass values from calling service, or create overloaded methods
   ```java
   // Provider service
   @GetMapping("/items")
   ResponseEntity<List<ItemDto>> getItems(@RequestParam int pageSize);
   
   // Calling service
   public List<ItemDto> getItems(Integer pageSize) {
       return client.getItems(pageSize != null ? pageSize : 10).getBody();
   }
   ```

### ❌ **Patterns to Avoid (Not Supported)**

1. **File Upload/Download Between Services**
   - Not supported in current SDK
   - Use dedicated file storage service (S3, MinIO) with URL/ID references
   ```java
   // Instead of uploading files directly
   @PostMapping("/documents/upload")
   ResponseEntity<DocumentDto> upload(@RequestPart MultipartFile file);
   
   // Use this pattern:
   @PostMapping("/documents")
   ResponseEntity<DocumentDto> createDocument(@RequestBody DocumentDto doc);
...
2. **Multiple Request Bodies**
   - Not supported and not RESTful
   - Use wrapper DTO instead
   ```java
   // DON'T DO THIS
   @PostMapping("/merge")
   ResponseEntity<UserDto> merge(@RequestBody UserDto user1, @RequestBody UserDto user2);
   
   // DO THIS
   @PostMapping("/merge")
   ResponseEntity<UserDto> merge(@RequestBody MergeRequest request);
...
3. **@RequestMapping with method Attribute**
   - Use specific annotations instead
   ```java
   // DON'T DO THIS
   @RequestMapping(value = "/users", method = RequestMethod.POST)
...
1. **API Design Principles**
   - Use `@GetMapping`, `@PostMapping`, etc. (not `@RequestMapping`)
   - Wrap return types in `ResponseEntity<T>`
   - Use DTOs for all request/response bodies (no primitives directly)
   - Avoid complex query parameters; use POST with request body for searches

2. **Security & Tracing**
   - Implement service authentication via interceptors (not per-method headers)
   - Use Spring Cloud Sleuth or similar for distributed tracing
   - Consider API Gateway for cross-cutting concerns

3. **File Handling**
   - Use object storage (S3, MinIO) for file transfers between services
   - Pass file URLs/keys in DTOs, not file content

4. **Resilience**
   - Leverage built-in Circuit Breaker (Resilience4j) - already configured
   - Leverage built-in Retry mechanism - already configured
   - Configure appropriate timeouts in RestClient

5. **Versioning**
   - Use header-based versioning (already supported via `ApiVersionHeaderGenerator`)
   - Or path-based versioning with `/v1/`, `/v2/` prefixes (supported)

### 📋 **Pre-Development Checklist**

Before designing a new microservice API that will use this SDK:

- [ ] All endpoints use specific HTTP method annotations (`@GetMapping`, `@PostMapping`, etc.)
- [ ] All methods return `ResponseEntity<T>` (not direct types)
- [ ] No `@RequestHeader` parameters (handle via interceptors)
- [ ] No file uploads/downloads (use object storage instead)
- [ ] No `@RequestParam List<?>` (use POST with body or comma-separated strings)
- [ ] No multiple `@RequestBody` parameters (use wrapper DTO)
- [ ] Path variables are always required (no optional path segments)
- [ ] Complex search/filter operations use POST with request body
...
- **Verification**: All statuses verified against actual implementation in `bits-sdk-processor` module
